...
test('release card keeps the chosen pass forecast for one second', () => {
  ...
  const card=releaseCard(play,passes,34);
...
test('pass recap freezes the chosen receiver forecast before release and expires', () => {
  ...
  assert.deepEqual(actionRecap(play,passes,1,20),['1.14','PASS']);
